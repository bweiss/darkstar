...
alias status (args) {
	...
	if (quiet) {
		...
		switch ($status.change($sbar)) {
			...
			(1) { echo Error: status.change\(\): Status bar not found: $sbar; }
			(*) { echo Error: status.change\(\): Unknown; }
		}
	}
}
	...
	for dir in ($DS.STATUS_DIR) {
		...
		if (fexist($dir) == 1) {
			for file in ($glob($dir\/\*)) {
				^local lastchr $mid(${strlen($file) - 1} 1 $file)
				unless (lastchr == [/]) {
					^local name $after(-1 / $file)
					if ((:item = finditem(status $file)) > -1) {
						@ delitem(status $item)
						@ delitem(status_files $item)
					}
