...
/*
 * Writes format settings to $DS.USER_DIR/themes/custom/
 * Returns 0 on success and > 0 on failure.
 */
...
/*
 * Write STATUS* /SETs to $DS.USER_DIR/status/custom
 * Returns 0 on success and > 0 on failure.
 */
alias save.write_status (void)
{
	@:fname = DS.USER_DIR ## [/status/custom]
	@ unlink($fname)
	@:fd = open($fname W)
	...
	@:setvars = [INPUT_PROMPT REVERSE_STATUS_LINE ] ## getsets(STATUS*)

	@ write($fd # Generated by DarkStar $DS.VERSION \($DS.INTERNAL_VERSION\): $strftime(%c))
	@ write($fd)
	@ write($fd assign STATUS.DOUBLE $STATUS.DOUBLE)
	@ write($fd)
	for var in ($setvars) {
